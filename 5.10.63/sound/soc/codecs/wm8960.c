...
SOC_SINGLE("MICB", WM8960_POWER1, 1, 1, 0),

SOC_SINGLE("Left ADC", WM8960_POWER1, 3, 1, 0),
		...
		unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBM_CFM:
		printk("codec fmt is master");
		iface |= 0x0040;
		break;
	case SND_SOC_DAIFMT_CBS_CFS:
		printk("codec fmt is slave");
		break;
	default:
		return -EINVAL;
	}
	...
	case SND_SOC_DAIFMT_NB_NF:
		printk("codec fmt is SND_SOC_DAIFMT_NB_NF");
		break;
...
static int wm8960_configure_clocking(struct snd_soc_component *component)
{
	...
	if (wm8960->clk_id == WM8960_SYSCLK_AUTO) {
		...
		freq_out = freq_in;
		dev_err(component->dev, "use MCLK, out=in=%d\n", freq_in);
			    ...
			    struct snd_soc_dai *dai)
{
	...
	dev_info(component->dev, "%s| bclk=%d, lrclk=%d\n", 
				__func__, wm8960->bclk, params_rate(params));
	/* bit size */
...
{
	struct snd_soc_component *component = dai->component;
	dev_info(component->dev, "%s| set %s\n", __func__, mute? "mute":"unmute");
	if (mute)
		...
		unsigned int freq_in, unsigned int freq_out)
{
	...
	int ret;
	printk("%s|set freq_in=%d, freq_out=%d\n",
		...
		int source, unsigned int freq_in, unsigned int freq_out)
{
	struct snd_soc_component *component = codec_dai->component;
	struct wm8960_priv *wm8960 = snd_soc_component_get_drvdata(component);
	dev_info(component->dev, "%s | set dai_pll=%d, freq_in=%d, freq_out=%d\n",
		__func__, pll_id, freq_in,freq_out);
	wm8960->freq_in = freq_in;

	if (pll_id == WM8960_SYSCLK_AUTO) {
		dev_info(component->dev,"pll set as WM8960_SYSCLK_AUTO\n");
		return 0;
	}
		...
		int div_id, int div)
{
	struct snd_soc_component *component = codec_dai->component;
	u16 reg;
	dev_info(component->dev, "%s | set clkdiv id=%d, div=%d\n", __func__, div_id, div);
	switch (div_id) {
					...
					unsigned int freq, int dir)
{
	...
	dev_info(component->dev, "%s | set clk_id=%d, freq=%d\n", __func__, clk_id, freq);
	switch (clk_id) {
	...
	case WM8960_SYSCLK_AUTO:
		dev_info(component->dev, "set as SYSCLK_AUTO");
		break;
	default:
		return -EINVAL;
	}
