			 ...
			 struct snd_soc_dai_link *dai_link)
{
	...
	dev_info(card->dev, "ASoC: failed to find rtd %s\n", dai_link->name);
	return NULL;
}
...
static void soc_resume_deferred(struct work_struct *work)
{
	...
	dev_info(card->dev, "ASoC: starting resume work\n");
	...
	dev_info(card->dev, "ASoC: resume work completed\n");
...
int snd_soc_resume(struct device *dev)
{
	...
	dev_info(dev, "ASoC: Scheduling resume work\n");
...
int snd_soc_unregister_card(struct snd_soc_card *card)
{
	...
	mutex_unlock(&client_mutex);
	dev_info(card->dev, "ASoC: Unregistered card '%s'\n", card->name);
...
{
	dev_info(dai->dev, "ASoC: Unregistered DAI '%s'\n", dai->name);
	list_del(&dai->list);
}
					 ...
					 bool legacy_dai_naming)
{
	...
	dev_info(dev, "ASoC: dynamically register DAI %s\n", dev_name(dev));
	...
	dev_info(dev, "ASoC: Registered DAI '%s'\n", dai->name);
	return dai;
}
			  ...
			  int num_dai)
{
	int ret;
	int i;
	dev_info(component->dev,"%s| add %s, drv_name(%s)\n",
			__func__, component->name, component->driver->name);
