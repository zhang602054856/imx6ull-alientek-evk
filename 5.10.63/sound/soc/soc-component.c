				    ...
				    unsigned int reg)
{
	...
	//dev_info(component->dev,"%s|reg(0x%x), val=0x%x\n", __func__, reg, val);
				  ...
				  unsigned int reg, unsigned int mask, unsigned int val)
{
	...
	if (component->regmap)
		...
		ret = snd_soc_component_update_bits_legacy(component, reg,
							   mask, val, &change);
	//dev_info(component->dev,"%s|reg(%x), mask=0x%x,val=0x%x\n", __func__, reg, mask, val);
					...
					unsigned int reg, unsigned int mask, unsigned int val)
{
	...
	if (component->regmap)
		...
		ret = snd_soc_component_update_bits_legacy(component, reg,
							   mask, val, &change);
	//dev_info(component->dev,"%s|reg(%x), val = %x\n", __func__, reg, val);
