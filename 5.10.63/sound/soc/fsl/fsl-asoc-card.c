				   ...
				   struct snd_pcm_hw_params *params)
{
	...
	dev_info(dev, "%s | sample_rate=%d, format=%d\n",
			__func__, priv->sample_rate, priv->sample_format);
...
static int fsl_asoc_card_probe(struct platform_device *pdev)
{
	...
	if (codec_np) {
		...
		if (codec_i2c) {
			...
			dev_info(&pdev->dev, "case: found codec in i2c device\n");
		}
		if (!codec_dev) {
			...
			dev_err(&pdev->dev, "case: Can not found codec in i2c device\n");
		}
	}
	...
	if (codec_dev) {
		...
		if (!IS_ERR(codec_clk)) {
			...
			printk("%s | codec_mclk_freq=%ld\n", __func__, priv->codec_priv.mclk_freq);
			clk_put(codec_clk);
	...
	if (asrc_pdev) {
		...
		priv->dai_link[2].codecs->name = priv->dai_link[0].codecs->name;
